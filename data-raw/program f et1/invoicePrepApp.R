...
server <- function(input, output, session) {
  ...
  inactivity_limit <- 60 * 2  # 5 minutes in seconds

  # Reactive timer that checks inactivity
  timer <- reactiveVal(Sys.time())

  # Check inactivity every second
  observe({
    invalidateLater(1000, session)  # Update every second
    time_elapsed <- as.numeric(difftime(Sys.time(), timer(), units = "secs"))
    if (time_elapsed > inactivity_limit) {
      stopApp()  # Shut down Shiny app
      # system("kill $(lsof -t -i:3838)")  # Forcefully close local server on port 3838
    }

    # output$time_left <- renderText({
    #   paste("Time until shutdown:", max(inactivity_limit - time_elapsed, 0), "seconds")
    # })
  })
  ...
  formatted_data <- reactive({
    ...
    # Check if the "Klinik" column in Avläsningar exists in Kundregister
    diff <- indataa$Avläsningar %>%
      anti_join(indataa$kundregister, by = "Klinik") %>%
      select(Klinik)
    ...
    if (any(nrow(diff)) > 0) {
      ...
      return(
        indataa$Avläsningar %>%
          inner_join(indataa$kundregister, by = "Klinik") %>%
